...
> **Authors**: Hadi Hajieghrary, Benedikt Walter, Chaitanya Shinde, Miguel Hurtado — TORC Robotics LLC
...
**Stage 1 results:** Applicability head converges to best validation loss 0.2772 (epoch 17) with F1=0.818, precision=0.708, recall=0.948. Per-tier accuracy: Legal and Road tiers stabilize near 1.0 while Safety stabilizes near 0.6 — reflecting the higher difficulty of safety-rule applicability detection due to class imbalance.

**Stage 2 results:** Full model converges to best validation loss 2.3487 (epoch 19). Validation ADE drops from 3.27m to 0.88m; FDE from 6.67m to 1.52m. The spike at epoch 6 reflects a transient instability during OneCycleLR warmup, self-corrected by epoch 8. Continuation training (3 additional epochs at LR=5e-5) reduces ADE to 0.67m.

**Learning rate schedules:** Stage 1 OneCycleLR peaks at 3e-4. Stage 2 OneCycleLR peaks at 1.5e-4 (the encoder receives 0.1x of these values). Training curve figures are generated by running [`generate_all_artifacts.py`](models/RECTOR/scripts/artifacts/README.md).

Training pipeline details: [`models/RECTOR/scripts/training/README.md`](models/RECTOR/scripts/training/README.md).

### Inference Pipeline

The inference pipeline ([`models/RECTOR/scripts/inference/pipeline.py`](models/RECTOR/scripts/inference/pipeline.py)) executes five stages:
...
Total inference latency: **7.3 ms** (scene encoding 2.1ms, trajectory generation 3.8ms, rule evaluation 1.4ms). Throughput: ~98 samples/second. Latency breakdown: trajectory generation dominates at 52.1% (3.8ms); rule evaluation adds 19.2% (1.4ms) — the cost of compliance awareness. RECTOR (98 samples/s) is competitive with MTR (95) and Wayformer (85), and moderately below DenseTNT (120) and LaneGCN (180). Efficiency figures are generated by running [`generate_all_artifacts.py`](models/RECTOR/scripts/artifacts/README.md).
