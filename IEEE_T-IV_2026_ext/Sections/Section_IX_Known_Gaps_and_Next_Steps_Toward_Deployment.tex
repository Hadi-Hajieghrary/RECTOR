...
The current WOMD benchmark is already largely rule-abiding, so the remaining headroom for absolute violation reduction is limited.  In that setting, the main value of RECTOR is not broad correction of pervasive violations but safer selection among a fixed candidate set, especially in the rare high-risk cases that remain after generation.
...
The second priority is to improve the quality of the trajectories being ranked and the quality of the rule-activation signal used to rank them.  The conservative Safety/Legal activation study, oracle-applicability cross-check, and selector ablations already identify the main leverage points: better recall on high-priority rules, stronger calibration for moderate-support rules, and generators that place more candidate mass inside the compliant region of trajectory space.  Because the benchmark is already mostly compliant, these gains should be interpreted as reducing the residual tail of unsafe candidates rather than as fixing a broadly non-compliant corpus.

\subsubsection{Extend the Planning Horizon}
The third priority is to move beyond a single 5-second open-loop decision.  Strategic positioning, longer merges, and lane-preparation behaviors require either hierarchical planning or receding-horizon replanning.  One practical next step is to integrate RECTOR's rule evaluation and lexicographic selection with a pretrained M2I DenseTNT stack so that longer-horizon replanning can be studied directly in the main experimental workflow.
