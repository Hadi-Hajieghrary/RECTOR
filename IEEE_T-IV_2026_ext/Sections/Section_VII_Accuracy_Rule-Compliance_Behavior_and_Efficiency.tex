...
Because the benchmark candidate sets are already mostly compliant, the compliance numbers below should be read as residual-risk reductions on the executed mode rather than as evidence of widespread rule breaking in WOMD.  The main question is whether the selector can remove the minority of Safety-heavy tail cases while leaving the underlying candidate distribution fixed.
...
This experiment isolates the paper's central empirical claim: when the candidate set is held fixed and already mostly compliant, selection can still reduce the residual rule-violating tail in the executed trajectory.  We apply three strategies to the \emph{same} candidate set~$\mathcal{T}(s)$---Confidence Only ($\arg\max_k p^{(k)}$), Weighted Sum ($\arg\min_k \sum_r w_r \bar{V}_{r,k}$), and RECTOR (lexicographic)---and evaluate all three under Protocol~A on 12{,}800 scenarios.  Table~\ref{tab:selection_strategy} reports the results.
...
\end{subfigure}
\caption{Rule-aware selection comparison.  \textbf{(a)}~Accuracy--compliance Pareto front: both rule-aware strategies achieve lower residual selected-trajectory violation rates than confidence-only selection on the same candidate set.  \textbf{(b)}~Per-tier breakdown: Safety-tier reduction is the primary differentiator, while weighted-sum and RECTOR are empirically identical in per-tier compliance on this benchmark.}
...
\caption{Residual selected-mode violation reduction summary (Protocol~A, same candidate set).  Total violations fall by 8.83~pp (23.86\%~$\rightarrow$~15.03\%), equivalently raising no-violation rate from 76.14\% to 84.97\%, driven primarily by Safety-tier improvement ($-$8.72~pp, 21.87\%~$\rightarrow$~13.15\%).  Legal-tier violations remain 0.0\% across all strategies because Protocol~A suppresses that tier.}
...
\end{tabular}
\vspace{1mm}
\caption*{\footnotesize Oracle applicability activates a broader set of rules than the learned head and therefore reports higher violation rates across Safety, Legal, Road, and Comfort.  The larger numbers reflect wider audit coverage, not a benchmark dominated by violations.  The largest gap is in Safety (13.15\%$\to$19.94\%), followed by Comfort (2.07\%$\to$11.97\%).}
\end{table}
...
\end{tabular}
\vspace{1mm}
\caption*{\footnotesize $^{\dagger}$Hybrid conservative: always-on for Tier~0 (Safety) and Tier~1 (Legal), learned predictions for Tier~2 (Road) and Tier~3 (Comfort).  Each row uses the specified applicability mode for selection, then all rows are evaluated under oracle applicability (full-28 rules).  The learned head produces 2.64~pp higher Safety-tier violations than both conservative policies (23.06\% vs.\ 20.43\%).  The hybrid and always-on policies are close to oracle Safety performance (20.43\% vs.\ 20.36\%), while oracle selection yields the lowest Total violations (32.67\%).  The main practical effect is thus safer selection in the tail, not a large shift in average driving behaviour.}
\end{table}
...
\end{tabular}
\vspace{1mm}
\caption*{\footnotesize Protocol~A ablation on the full augmented validation set, using the same protocol as all main results (Tables~\ref{tab:selection_strategy}--\ref{tab:per_tier_breakdown}).  Rule-aware selection reduces Safety violations by $-$8.70~pp (22.09\%\,$\to$\,13.39\%) and Total violations by $-$8.88~pp (24.06\%\,$\to$\,15.18\%) relative to confidence-only selection, equivalently raising selected-mode compliance from 75.94\% to 84.82\% on the augmented split.  Lexicographic and weighted-sum achieve identical compliance; the 0.090\,m selADE gap reflects the confidence tiebreaker in RECTOR's lexicographic selector.  At $n{=}43{,}219$, the 8.70~pp Safety reduction is $>$40 standard errors.  This is a fixed-candidate-set effect on the selected mode, not a claim that the benchmark distribution itself changes.}
\end{table}
...
The test-set geometric metrics degrade substantially relative to validation (minADE: 0.684\,m $\to$ 2.86\,m, +318\%; Miss Rate: 18.56\% $\to$ 52.2\%, +181\%), which reinforces the point that the selector's compliance gains should be interpreted as operating on a specific held-out distribution rather than as a universal property of all WOMD splits.
