...
DATA_DIR="${WAYMO_RAW_ROOT:-/workspace/data/WOMD/datasets/waymo_open_dataset/motion_v_1_3_0/raw}"
...
mkdir -p "${DATA_DIR}"/{scenario,tf,lidar_and_camera}/{training,validation,testing}
...
download_sample() {
    ...
    local split=$2
    local num_files=$3
    local dir_format=$format

    # Map tf_example -> tf for local directory structure
    if [ "$format" == "tf_example" ]; then
        dir_format="tf"
    fi

    local source="${BUCKET}/${format}/${split}"
    local dest="${DATA_DIR}/${dir_format}/${split}"
...
# Optional: Download TF format (can be generated from scenario, legacy support)
if [ "${DOWNLOAD_TF:-0}" != "0" ]; then
    echo ""
    print_step "Downloading TF format (optional - can be converted from scenario)..."
    ...
    echo ""
    print_info "Skipping TF format download (can be generated from scenario)"
    print_info "To download TF format: DOWNLOAD_TF=1 $0"
fi
...
echo "  NUM_SAMPLE_FILES=20 bash scripts/WOMD/download_waymo_sample.sh"
echo ""
echo "To include interactive scenarios (validation_interactive, testing_interactive):"
echo "  DOWNLOAD_INTERACTIVE=1 bash scripts/WOMD/download_waymo_sample.sh"
echo ""
echo "To include 20-second extended training scenarios:"
echo "  DOWNLOAD_20S=1 bash scripts/WOMD/download_waymo_sample.sh"
echo ""
echo "To include lidar/camera data:"
echo "  DOWNLOAD_LIDAR=1 bash scripts/WOMD/download_waymo_sample.sh"
echo ""
