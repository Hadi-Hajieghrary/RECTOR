...
\noindent Protocol~B applies the full rule set with oracle applicability, so its absolute violation rates are higher than Protocol~A and are reported as a separate evaluation setting.
...
\textbf{RECTOR (Ours):}  Transformer--CVAE with rule-aware lexicographic selection, trained via two-stage procedure (Section~\ref{subsec:system_overview}).  Reported metrics: minADE~= 0.684\,m, minFDE~= 1.270\,m, Miss Rate~= 18.56\%, throughput~= 136.5~samples/s.  Compliance improvement (Protocol~A): total violations 23.86\%~$\rightarrow$~15.03\% ($-$8.83~pp), Safety-tier violations 21.87\%~$\rightarrow$~13.15\% ($-$8.72~pp), Legal-tier violations 0.0\% across all strategies vs.\ confidence-only selection.
